...
- 用户头像上传与服务端方形裁剪、站内预设头像选择
- 摄影作品发布、多图上传、编辑、删除
- 用户可设置新作品默认可见度，并可对单篇作品设置公开、仅自己可见、关注后可见与 0–7 天关注等待期
- 点赞、收藏、评论、关注
- 用户主页与联系方式打码
...
包含首页 Banner、登录背景、社区封面、活动封面、默认头像、空状态插画。用户资料页另提供首批 10 张本地预设头像。详细提示词见 `docs/image_prompts.md`。管理员可在后台“网站设置”中上传新图片替换。
...
项目还提供安全回归测试，覆盖 CSRF、开放重定向、上传伪装、像素限制、头像裁剪、预设头像白名单、作品可见度、私有图片授权、管理员越权、会话失效、XSS、CSV 公式注入、限流和审计日志来源等场景：

```powershell
python scripts/security_test.py
```

头像裁剪与作品可见度还有一组独立回归测试：

```powershell
python scripts/visibility_test.py
```
