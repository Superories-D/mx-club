...
2. 系统生成 CSV，左侧是序号和“真实姓名（填写）”，右侧是“邀请码（剪下发放）”。
3. 负责老师按顺序填写真实姓名，右侧邀请码可剪切分发给对应同学。
4. 管理员上传填好姓名后的 CSV，系统自动按每行真实姓名和邀请码配对。

空白表字段如下：

```csv
序号,真实姓名（填写）,邀请码（剪下发放）,备注
1,张三,MUXI2026A001,右侧邀请码可剪下分发；填好姓名后上传本表自动配对
```
